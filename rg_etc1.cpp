...
constexpr u32 cUINT32_MAX = std::numeric_limits<u32>::max();
constexpr u64 cUINT64_MAX = std::numeric_limits<u64>::max();

template<typename T> constexpr T minimum(T a, T b) { return (a < b) ? a : b; }
template<typename T> constexpr T minimum(T a, T b, T c) { return minimum(minimum(a, b), c); }
template<typename T> constexpr T maximum(T a, T b) { return (a > b) ? a : b; }
template<typename T> constexpr T maximum(T a, T b, T c) { return maximum(maximum(a, b), c); }
...
template<typename T> void zero_object(T& obj) { std::memset(&obj, 0, sizeof(obj)); }

enum eNoClamp { cNoClamp };

struct color_quad_u8
{
    static inline int clamp(int v) { if (v & 0xFFFFFF00U) { v = (~(static_cast<int>(v) >> 31)) & 0xFF; 
}return v; }
...
public:
    ...
    inline color_quad_u8(const color_quad_u8& other) = default;
    ...
    inline color_quad_u8& operator= (const color_quad_u8& other) = default;
    ...
    inline component_t operator[] (u32 i) const { RG_ETC1_ASSERT(i < cNumComps); return c[i]; }
    inline component_t& operator[] (u32 i) { RG_ETC1_ASSERT(i < cNumComps); return c[i]; }

    inline color_quad_u8& set_component(u32 i, parameter_t f)
    {
    ...
    {
        auto x = static_cast<component_t>(clamp(l));
        c[0] = x;
        c[1] = x;
        c[2] = x;
        return *this;
    ...
    inline color_quad_u8& clamp(parameter_t l, parameter_t h)
    {
        for (unsigned char & i : c) {
            i = static_cast<component_t>(rg_etc1::clamp<parameter_t>(i, l, h));
}
        return *this;
    }
    ...
    inline u32 squared_distance_rgb(const color_quad_u8& c) const
    {
        return rg_etc1::square(r - c.r) + rg_etc1::square(g - c.g) + rg_etc1::square(b - c.b);
    }

    inline u32 squared_distance_rgba(const color_quad_u8& c) const
    {
...
struct vec3F
{
    ...
    inline float operator[] (u32 i) const { RG_ETC1_ASSERT(i < 3); return m_s[i]; }

    inline vec3F& operator += (const vec3F& other) { for (u32 i = 0; i < 3; i++) { m_s[i] += other.m_s[i]; 
}return *this; }

    inline vec3F& operator *= (float s) { for (float & m_ : m_s) { m_ *= s; 
}return *this; }
};
...
static std::array<u8, 256 + 16> g_quant5_tab; // NOLINT(cppcoreguidelines-avoid-non-const-global-variables) // populated at runtime by pack_etc1_block_init()

static const std::array<std::array<s32, cETC1SelectorValues>, cETC1IntenModifierValues> g_etc1_inten_tables = {{
        {{ -8,  -2,   2,   8 }}, {{ -17,  -5,  5,  17 }}, {{ -29,  -9,   9,  29 }}, {{  -42, -13, 13,  42 }},
        {{ -60, -18, 18,  60 }}, {{ -80, -24, 24,  80 }}, {{ -106, -33, 33, 106 }}, {{ -183, -47, 47, 183 }}
    }};

static const std::array<u8, cETC1SelectorValues> g_etc1_to_selector_index = { 2, 3, 1, 0 };
static const std::array<u8, cETC1SelectorValues> g_selector_index_to_etc1 = { 3, 2, 0, 1 };
...
static std::array<std::array<u16, 256>, static_cast<std::size_t>(2) * 8 * 4> g_etc1_inverse_lookup;      // [diff/inten_table/selector][desired_color] // NOLINT(cppcoreguidelines-avoid-non-const-global-variables) // populated at runtime by pack_etc1_block_init()
...
static const u16 g_color8_to_etc_block_config_0_255[2][33] =
    {
...
// Really only [254][11].
static const u16 g_color8_to_etc_block_config_1_to_254[254][12] =
    {
...
struct etc1_block
{
    ...
    {
        u64 m_uint64;
        std::array<u8, 8> m_bytes;
    };

    std::array<u8, 2> m_low_color;
    std::array<u8, 2> m_high_color;

    static constexpr u32 cNumSelectorBytes = 4;
    std::array<u8, cNumSelectorBytes> m_selectors;
    ...
    inline u32 get_byte_bits(u32 ofs, u32 num) const
    {
        RG_ETC1_ASSERT((ofs + num) <= 64U);
        RG_ETC1_ASSERT(num && (num <= 8U));
        RG_ETC1_ASSERT((ofs >> 3) == ((ofs + num - 1) >> 3));
        const u32 byte_ofs = 7 - (ofs >> 3);
        const u32 byte_bit_ofs = ofs & 7;
        return (m_bytes[byte_ofs] >> byte_bit_ofs) & ((1 << num) - 1);
    }

    inline void set_byte_bits(u32 ofs, u32 num, u32 bits)
    {
        RG_ETC1_ASSERT((ofs + num) <= 64U);
    ...
    {
        m_bytes[3] &= ~2;
        m_bytes[3] |= (static_cast<u32>(diff) << 1);
    }

    // Returns intensity modifier table (0-7) used by subblock subblock_id.
    // subblock_id=0 left/top (CW 1), 1=right/bottom (CW 2)
    inline u32 get_inten_table(u32 subblock_id) const
    {
        RG_ETC1_ASSERT(subblock_id < 2);
        const u32 ofs = subblock_id ? 2 : 5;
        return (m_bytes[3] >> ofs) & 7;
    }

    // Sets intensity modifier table (0-7) used by subblock subblock_id (0 or 1)
    inline void set_inten_table(u32 subblock_id, u32 t)
    {
        RG_ETC1_ASSERT(subblock_id < 2);
        RG_ETC1_ASSERT(t < 8);
        const u32 ofs = subblock_id ? 2 : 5;
    ...
    inline u32 get_selector(u32 x, u32 y) const
    {
        RG_ETC1_ASSERT((x | y) < 4);

        const u32 bit_index = x * 4 + y;
        const u32 byte_bit_ofs = bit_index & 7;
        const u8 *p = &m_bytes[7 - (bit_index >> 3)];
        const u32 lsb = (p[0] >> byte_bit_ofs) & 1;
        const u32 msb = (p[-2] >> byte_bit_ofs) & 1;
    ...
    inline void set_selector(u32 x, u32 y, u32 val)
    {
        RG_ETC1_ASSERT((x | y | val) < 4);
        const u32 bit_index = x * 4 + y;

        u8 *p = &m_bytes[7 - (bit_index >> 3)];

        const u32 byte_bit_ofs = bit_index & 7;
        const u32 mask = 1 << byte_bit_ofs;

        const u32 etc1_val = g_selector_index_to_etc1[val];

        const u32 lsb = etc1_val & 1;
        const u32 msb = etc1_val >> 1;
    ...
    inline void set_base4_color(u32 idx, u16 c)
    {
        if (idx)
        {
    ...
    inline u16 get_base4_color(u32 idx) const
    {
        u32 r, g, b;
        if (idx)
        {
    ...
    inline void set_base5_color(u16 c)
    {
    ...
    inline u16 get_base5_color() const
    {
        const u32 r = get_byte_bits(cETC1BaseColor5RBitOffset, 5);
        const u32 g = get_byte_bits(cETC1BaseColor5GBitOffset, 5);
        const u32 b = get_byte_bits(cETC1BaseColor5BBitOffset, 5);
        return static_cast<u16>(b | (g << 5U) | (r << 10U));
    }

    void set_delta3_color(u16 c)
    {
    ...
    inline u16 get_delta3_color() const
    {
        const u32 r = get_byte_bits(cETC1DeltaColor3RBitOffset, 3);
        const u32 g = get_byte_bits(cETC1DeltaColor3GBitOffset, 3);
        const u32 b = get_byte_bits(cETC1DeltaColor3BBitOffset, 3);
        return static_cast<u16>(b | (g << 3U) | (r << 6U));
    }

    // Base color 5
    static u16 pack_color5(const color_quad_u8& color, bool scaled, u32 bias = 127U);
    static u16 pack_color5(u32 r, u32 g, u32 b, bool scaled, u32 bias = 127U);

    static color_quad_u8 unpack_color5(u16 packed_color5, bool scaled, u32 alpha = 255U);
    static void unpack_color5(u32& r, u32& g, u32& b, u16 packed_color, bool scaled);

    static bool unpack_color5(color_quad_u8& result, u16 packed_color5, u16 packed_delta3, bool scaled, u32 alpha = 255U);
    static bool unpack_color5(u32& r, u32& g, u32& b, u16 packed_color5, u16 packed_delta3, bool scaled, u32 alpha = 255U);

    // Delta color 3
    // Inputs range from -4 to 3 (cETC1ColorDeltaMin to cETC1ColorDeltaMax)
    static u16 pack_delta3(int r, int g, int b);

    // Results range from -4 to 3 (cETC1ColorDeltaMin to cETC1ColorDeltaMax)
    static void unpack_delta3(int& r, int& g, int& b, u16 packed_delta3);

    // Abs color 4
    static u16 pack_color4(const color_quad_u8& color, bool scaled, u32 bias = 127U);
    static u16 pack_color4(u32 r, u32 g, u32 b, bool scaled, u32 bias = 127U);

    static color_quad_u8 unpack_color4(u16 packed_color4, bool scaled, u32 alpha = 255U);
    static void unpack_color4(u32& r, u32& g, u32& b, u16 packed_color4, bool scaled);

    // subblock colors
    static void get_diff_subblock_colors(color_quad_u8* pDst, u16 packed_color5, u32 table_idx);
    static bool get_diff_subblock_colors(color_quad_u8* pDst, u16 packed_color5, u16 packed_delta3, u32 table_idx);
    static void get_abs_subblock_colors(color_quad_u8* pDst, u16 packed_color4, u32 table_idx);
...
T* indirect_radix_sort(u32 num_indices, T* pIndices0, T* pIndices1, const Q* pKeys, u32 key_ofs, u32 key_size, bool init_indices)
{
    ...
    std::memset(hist.data(), 0, sizeof(hist[0]) * 256 * key_size);

#define RG_ETC1_GET_KEY(p) (*(const u32*)((const u8*)(pKeys + *(p)) + key_ofs))
#define RG_ETC1_GET_KEY_FROM_INDEX(i) (*(const u32*)((const u8*)(pKeys + (i)) + key_ofs))
    ...
    for (u32 pass = 0; pass < key_size; pass++)
    {
        const u32* pHist = hist.data() + (static_cast<size_t>(pass) << 8);
        ...
        for ( ; p != q; p += 2)
        {
            ...
            u32 c0 = (RG_ETC1_GET_KEY_FROM_INDEX(index0) >> pass_shift) & 0xFF;
            u32 c1 = (RG_ETC1_GET_KEY_FROM_INDEX(index1) >> pass_shift) & 0xFF;
        ...
        {
            u32 index = *p;
            u32 c = (RG_ETC1_GET_KEY_FROM_INDEX(index) >> pass_shift) & 0xFF;
...
u16 etc1_block::pack_color5(const color_quad_u8& color, bool scaled, u32 bias)
{
    return pack_color5(color.r, color.g, color.b, scaled, bias);
}

u16 etc1_block::pack_color5(u32 r, u32 g, u32 b, bool scaled, u32 bias)
{
    if (scaled)
    {
        r = (r * 31U + bias) / 255U;
...
color_quad_u8 etc1_block::unpack_color5(u16 packed_color5, bool scaled, u32 alpha)
{
    u32 b = packed_color5 & 31U;
    u32 g = (packed_color5 >> 5U) & 31U;
    u32 r = (packed_color5 >> 10U) & 31U;
    ...
    return {cNoClamp, static_cast<int>(r), static_cast<int>(g), static_cast<int>(b), static_cast<int>(rg_etc1::minimum(alpha, 255U))};
}

void etc1_block::unpack_color5(u32& r, u32& g, u32& b, u16 packed_color5, bool scaled)
{
...
bool etc1_block::unpack_color5(color_quad_u8& result, u16 packed_color5, u16 packed_delta3, bool scaled, u32 alpha)
{
    int dc_r, dc_g, dc_b;
    unpack_delta3(dc_r, dc_g, dc_b, packed_delta3);

    int b = static_cast<int>(packed_color5 & 31U) + dc_b;
    int g = static_cast<int>((packed_color5 >> 5U) & 31U) + dc_g;
    int r = static_cast<int>((packed_color5 >> 10U) & 31U) + dc_r;
    ...
    result.set_noclamp_rgba(r, g, b, static_cast<int>(rg_etc1::minimum(alpha, 255U)));
    return success;
}

bool etc1_block::unpack_color5(u32& r, u32& g, u32& b, u16 packed_color5, u16 packed_delta3, bool scaled, u32 alpha)
{
...
u16 etc1_block::pack_delta3(int r, int g, int b)
{
...
void etc1_block::unpack_delta3(int& r, int& g, int& b, u16 packed_delta3)
{
    r = (packed_delta3 >> 6) & 7;
    g = (packed_delta3 >> 3) & 7;
    b = packed_delta3 & 7;
...
u16 etc1_block::pack_color4(const color_quad_u8& color, bool scaled, u32 bias)
{
    return pack_color4(color.r, color.g, color.b, scaled, bias);
}

u16 etc1_block::pack_color4(u32 r, u32 g, u32 b, bool scaled, u32 bias)
{
    if (scaled)
    {
        r = (r * 15U + bias) / 255U;
...
color_quad_u8 etc1_block::unpack_color4(u16 packed_color4, bool scaled, u32 alpha)
{
    u32 b = packed_color4 & 15U;
    u32 g = (packed_color4 >> 4U) & 15U;
    u32 r = (packed_color4 >> 8U) & 15U;
...
void etc1_block::unpack_color4(u32& r, u32& g, u32& b, u16 packed_color4, bool scaled)
{
...
void etc1_block::get_diff_subblock_colors(color_quad_u8* pDst, u16 packed_color5, u32 table_idx)
{
...
bool etc1_block::get_diff_subblock_colors(color_quad_u8* pDst, u16 packed_color5, u16 packed_delta3, u32 table_idx)
{
    RG_ETC1_ASSERT(table_idx < cETC1IntenModifierValues);
    const int *pInten_modifer_table = &g_etc1_inten_tables[table_idx][0];
...
void etc1_block::get_abs_subblock_colors(color_quad_u8* pDst, u16 packed_color4, u32 table_idx)
{
    RG_ETC1_ASSERT(table_idx < cETC1IntenModifierValues);
    const int *pInten_modifer_table = &g_etc1_inten_tables[table_idx][0];
...
bool unpack_etc1_block(const void* pETC1_block, u32* pDst_pixels_rgba, bool preserve_alpha)
{
    auto* pDst = std::bit_cast<color_quad_u8*>(pDst_pixels_rgba);
    const etc1_block& block = *static_cast<const etc1_block*>(pETC1_block);

    const bool diff_flag = block.get_diff_bit();
    const bool flip_flag = block.get_flip_bit();
    const u32 table_index0 = block.get_inten_table(0);
    const u32 table_index1 = block.get_inten_table(1);

    std::array<color_quad_u8, 4> subblock_colors0;
    std::array<color_quad_u8, 4> subblock_colors1;
    bool success = true;

    if (diff_flag)
    {
        const u16 base_color5 = block.get_base5_color();
        const u16 delta_color3 = block.get_delta3_color();
        etc1_block::get_diff_subblock_colors(subblock_colors0.data(), base_color5, table_index0);

        if (!etc1_block::get_diff_subblock_colors(subblock_colors1.data(), base_color5, delta_color3, table_index1)) {
            success = false;
}
    }
    else
    {
        const u16 base_color4_0 = block.get_base4_color(0);
        etc1_block::get_abs_subblock_colors(subblock_colors0.data(), base_color4_0, table_index0);

        const u16 base_color4_1 = block.get_base4_color(1);
        etc1_block::get_abs_subblock_colors(subblock_colors1.data(), base_color4_1, table_index1);
    }
...
struct etc1_solution_coordinates
{
    ...
    inline etc1_solution_coordinates(u32 r, u32 g, u32 b, u32 inten_table, bool color4) :
        m_unscaled_color(static_cast<int>(r), static_cast<int>(g), static_cast<int>(b), 255),
        m_inten_table(inten_table),
        m_color4(color4)
    {
    }

    inline etc1_solution_coordinates(const color_quad_u8& c, u32 inten_table, bool color4) :
        m_unscaled_color(c),
        m_inten_table(inten_table),
        m_color4(color4)
    {
    ...
    inline void get_block_colors(color_quad_u8* pBlock_colors)
    {
        ...
        const int* pInten_table = g_etc1_inten_tables[m_inten_table].data();
...
{
public:
    etc1_optimizer(const etc1_optimizer&) = delete;
    etc1_optimizer& operator=(const etc1_optimizer&) = delete;

    etc1_optimizer() = default;
    ...
    struct params : etc1_pack_params
    {
        ...
        void clear_optimizer_params()
        {
            ...
            m_use_color4 = false;
            static const std::array<int, 1> s_default_scan_delta = { 0 };
            m_pScan_deltas = s_default_scan_delta.data();
            m_scan_delta_size = 1;
        ...
        u32 m_num_src_pixels = 0;
        const color_quad_u8* m_pSrc_pixels = nullptr;

        bool m_use_color4 = false;
        const int* m_pScan_deltas = nullptr;
        u32 m_scan_delta_size = 0;

        color_quad_u8 m_base_color5;
        bool m_constrain_against_base_color5 = false;
    };

    struct results
    {
        u64 m_error = cUINT64_MAX;
        color_quad_u8 m_block_color_unscaled;
        u32 m_block_inten_table = 0;
        u32 m_n = 0;
        u8* m_pSelectors = nullptr;
        bool m_block_color4 = false;
...
private:
    ...
    {
        potential_solution() : m_coords() 
        {
        }

        etc1_solution_coordinates  m_coords;
        std::array<u8, 8>        m_selectors;
        u64                     m_error{cUINT64_MAX};
        bool                       m_valid{false};
    ...
    vec3F m_avg_color;
    int m_br, m_bg, m_bb;
    std::array<u16, 8> m_luma;
    std::array<std::array<u32, 8>, 2> m_sorted_luma;
    const u32* m_pSorted_luma_indices;
    u32* m_pSorted_luma;

    potential_solution m_best_solution;
    potential_solution m_trial_solution;
    std::array<u8, 8> m_temp_selectors;
...
// software-pipelined. Fills pErrors[8] with the total squared RGB error per table and
// pSelectors[8*8] with the best selector index (0-3) per pixel per table (table-major:
// pSelectors[t*8 + c]). All variants must produce bit-identical results: the candidate colors are
...
using rg_etc1_evaluate_inten_tables_func = void (*)(const color_quad_u8*, const color_quad_u8&, u64*, u8*);

// Cached dispatch target, assigned by pack_etc1_block_init() so the inner evaluation has no
// dispatch/CPU-feature cost. get_evaluate_inten_tables_func() performs the one-time detection.
static rg_etc1_evaluate_inten_tables_func get_evaluate_inten_tables_func();

static rg_etc1_evaluate_inten_tables_func g_eval_inten_tables = nullptr; // NOLINT(cppcoreguidelines-avoid-non-const-global-variables) // cached dispatch target set by pack_etc1_block_init()

static inline rg_etc1_evaluate_inten_tables_func get_cached_eval_inten_tables()
{
    return g_eval_inten_tables ? g_eval_inten_tables : get_evaluate_inten_tables_func();
}

static u64 evaluate_inten_table_scalar(const color_quad_u8* pSrc_pixels, const color_quad_u8& base_color, u32 inten_table, u8* pSelectors)
{
    const int* pInten_table = g_etc1_inten_tables[inten_table].data();

    std::array<color_quad_u8, 4> block_colors;
    for (u32 s = 0; s < 4; s++)
    {
    ...
    for (u32 c = 0; c < 8; c++)
    {
        const color_quad_u8& src_pixel = pSrc_pixels[c];

        u32 best_selector_index = 0;
        u32 best_error = block_colors[0].squared_distance_rgb(src_pixel);
        for (u32 s = 1; s < 4; s++)
        {
            const u32 trial_error = block_colors[s].squared_distance_rgb(src_pixel);
        ...
        pSelectors[c] = static_cast<u8>(best_selector_index);
...
static void evaluate_inten_tables_scalar(const color_quad_u8* pSrc_pixels, const color_quad_u8& base_color, u64* pErrors, u8* pSelectors)
{
    for (u32 t = 0; t < cETC1IntenModifierValues; t++) {
        pErrors[t] = evaluate_inten_table_scalar(pSrc_pixels, base_color, t, pSelectors + static_cast<size_t>(t) * 8);
}
}
...
static void evaluate_inten_tables_ssse3(const color_quad_u8* pSrc_pixels, const color_quad_u8& base_color, u64* pErrors, u8* pSelectors)
{
    // 8 pixels of RGBA == 32 bytes == two unaligned 128-bit loads.
    const __m128i px01 = _mm_loadu_si128(std::bit_cast<const __m128i*>(&pSrc_pixels[0]));
    const __m128i px23 = _mm_loadu_si128(std::bit_cast<const __m128i*>(&pSrc_pixels[4]));
    ...
    const __m128i base16 = _mm_set_epi16(0, base_color.b, base_color.g, base_color.r, 0, base_color.b, base_color.g, base_color.r);
    ...
    for (u32 t = 0; t < cETC1IntenModifierValues; t++)
    {
        const int* pInten = g_etc1_inten_tables[t].data();

        __m128i cvec[4];
        for (u32 s = 0; s < 4; s++)
        {
            const __m128i ydv = _mm_set1_epi16(static_cast<short>(pInten[s]));
        ...
        for (u32 s = 1; s < 4; s++)
        {
            ...
            for (u32 v = 0; v < 4; v++)
            {
                ...
                const __m128i lt = _mm_cmpgt_epi32(best_err[v], e);
                best_err[v] = _mm_or_si128(_mm_and_si128(lt, e), _mm_andnot_si128(lt, best_err[v]));
                best_sel[v] = _mm_or_si128(_mm_and_si128(lt, seldv), _mm_andnot_si128(lt, best_sel[v]));
            }
        }

        u8* dst = pSelectors + static_cast<size_t>(t) * 8;
        for (u32 v = 0; v < 4; v++)
        {
            dst[static_cast<size_t>(v) * 2 + 0] = static_cast<u8>(static_cast<u32>(_mm_cvtsi128_si32(best_sel[v])) & 3U);
            dst[static_cast<size_t>(v) * 2 + 1] = static_cast<u8>(static_cast<u32>(_mm_cvtsi128_si32(_mm_shuffle_epi32(best_sel[v], 0x0E))) & 3U);
        }

        u64 total_error = 0;
        for (const auto& e : best_err)
        {
            const __m128i sum = _mm_add_epi32(e, _mm_shuffle_epi32(e, 0x0E));
            total_error += static_cast<u32>(_mm_cvtsi128_si32(sum));
        }
        pErrors[t] = total_error;
    }
    // NOLINTEND(cppcoreguidelines-avoid-c-arrays,modernize-avoid-c-arrays)
}
...
static void evaluate_inten_tables_avx2(const color_quad_u8* pSrc_pixels, const color_quad_u8& base_color, u64* pErrors, u8* pSelectors)
{
    const __m128i px01 = _mm_loadu_si128(std::bit_cast<const __m128i*>(&pSrc_pixels[0]));
    const __m128i px23 = _mm_loadu_si128(std::bit_cast<const __m128i*>(&pSrc_pixels[4]));
    ...
    const __m256i base256 = _mm256_set_epi16(0, base_color.b, base_color.g, base_color.r,
                                             0, base_color.b, base_color.g, base_color.r,
                                             0, base_color.b, base_color.g, base_color.r,
                                             0, base_color.b, base_color.g, base_color.r);
    ...
    for (u32 t = 0; t < cETC1IntenModifierValues; t++)
    {
        ...
        for (u32 s = 0; s < 4; s++)
        {
            const __m256i ydv = _mm256_set1_epi16(static_cast<short>(pInten[s]));
        ...
        for (u32 s = 1; s < 4; s++)
        {
            ...
            for (u32 v = 0; v < 2; v++)
            {
                ...
                const __m256i lt = _mm256_cmpgt_epi32(best_err[v], e);
                best_err[v] = _mm256_min_epi32(best_err[v], e);
                best_sel[v] = _mm256_blendv_epi8(best_sel[v], seldv, lt);
            }
        }
        ...
        for (u32 v = 0; v < 2; v++)
        {
            dst[static_cast<size_t>(v) * 4 + 0] = static_cast<u8>(static_cast<u32>(_mm256_extract_epi32(best_sel[v], 0)) & 3U);
            dst[static_cast<size_t>(v) * 4 + 1] = static_cast<u8>(static_cast<u32>(_mm256_extract_epi32(best_sel[v], 2)) & 3U);
            dst[static_cast<size_t>(v) * 4 + 2] = static_cast<u8>(static_cast<u32>(_mm256_extract_epi32(best_sel[v], 4)) & 3U);
            dst[static_cast<size_t>(v) * 4 + 3] = static_cast<u8>(static_cast<u32>(_mm256_extract_epi32(best_sel[v], 6)) & 3U);
        }
        ...
        for (const auto& e : best_err)
        {
            ...
            total_error += static_cast<u32>(_mm256_extract_epi32(sum, 0)) + static_cast<u32>(_mm256_extract_epi32(sum, 4));
        }
        pErrors[t] = total_error;
    }
    // NOLINTEND(cppcoreguidelines-avoid-c-arrays,modernize-avoid-c-arrays)
...
static rg_etc1_evaluate_inten_tables_func get_evaluate_inten_tables_func()
{
    static const rg_etc1_evaluate_inten_tables_func s_func = []() -> rg_etc1_evaluate_inten_tables_func
    {
#ifdef RG_ETC1_FORCE_SCALAR
        return &evaluate_inten_tables_scalar;
#else
#if defined(__GNUC__) || defined(__clang__)
        __builtin_cpu_init();
        if (__builtin_cpu_supports("avx2")) {
            return &evaluate_inten_tables_avx2;
}
        if (__builtin_cpu_supports("ssse3")) {
            return &evaluate_inten_tables_ssse3;
}
...
{
    const u32 n = m_pParams->m_num_src_pixels;
    const int scan_delta_size = static_cast<int>(m_pParams->m_scan_delta_size);
    ...
    for (int zdi = 0; zdi < scan_delta_size; zdi++)
    {
        ...
        if (mbb < 0) { continue; } if (mbb > m_limit) { break;
}
        ...
        for (int ydi = 0; ydi < scan_delta_size; ydi++)
        {
            ...
            if (mbg < 0) { continue; } if (mbg > m_limit) { break;
}
            ...
            for (int xdi = 0; xdi < scan_delta_size; xdi++)
            {
                ...
                if (mbr < 0) { continue; } if (mbr > m_limit) { break;
}

                etc1_solution_coordinates coords(mbr, mbg, mbb, 0, m_pParams->m_use_color4);
                if (m_pParams->m_quality == etc1_quality::High)
                {
                ...
                const u32 max_refinement_trials = (m_pParams->m_quality == etc1_quality::Low) ? 2 : (((xd | yd | zd) == 0) ? 4 : 2);
                for (u32 refinement_trial = 0; refinement_trial < max_refinement_trials; refinement_trial++)
                {
                    const u8* pSelectors = m_best_solution.m_selectors.data();
                    const int* pInten_table = g_etc1_inten_tables[m_best_solution.m_coords.m_inten_table].data();

                    int delta_sum_r = 0, delta_sum_g = 0, delta_sum_b = 0;
                    const color_quad_u8 base_color(m_best_solution.m_coords.get_scaled_color());
                    for (u32 r = 0; r < n; r++)
                    {
                        const u32 s = *pSelectors++;
                        const int yd = pInten_table[s];
                    ...
                    const float avg_delta_r_f = static_cast<float>(delta_sum_r) / static_cast<float>(n);
                    const float avg_delta_g_f = static_cast<float>(delta_sum_g) / static_cast<float>(n);
                    const float avg_delta_b_f = static_cast<float>(delta_sum_b) / static_cast<float>(n);
                    const auto limit_f = static_cast<float>(m_limit);
                    const auto br1 = rg_etc1::clamp<int>(static_cast<int>(std::lroundf((m_avg_color[0] - avg_delta_r_f) * limit_f / 255.0f)), 0, m_limit);
                    const auto bg1 = rg_etc1::clamp<int>(static_cast<int>(std::lroundf((m_avg_color[1] - avg_delta_g_f) * limit_f / 255.0f)), 0, m_limit);
                    const auto bb1 = rg_etc1::clamp<int>(static_cast<int>(std::lroundf((m_avg_color[2] - avg_delta_b_f) * limit_f / 255.0f)), 0, m_limit);

                    const bool skip = ((mbr == br1) && (mbg == bg1) && (mbb == bb1))
                                    || ((br1 == m_best_solution.m_coords.m_unscaled_color.r) && (bg1 == m_best_solution.m_coords.m_unscaled_color.g) && (bb1 == m_best_solution.m_coords.m_unscaled_color.b))
                                    || ((m_br == br1) && (m_bg == bg1) && (m_bb == bb1));
...
void etc1_optimizer::init(const params& p, results& r)
{
    ...
    for (u32 i = 0; i < n; i++)
    {
        ...
        m_luma[i] = static_cast<u16>(c.r + c.g + c.b);
        m_sorted_luma[0][i] = i;
    }
    ...
    const auto limit_f = static_cast<float>(m_limit);
    m_br = rg_etc1::clamp<int>(static_cast<int>(std::lroundf(m_avg_color[0] * limit_f / 255.0f)), 0, m_limit);
    m_bg = rg_etc1::clamp<int>(static_cast<int>(std::lroundf(m_avg_color[1] * limit_f / 255.0f)), 0, m_limit);
    m_bb = rg_etc1::clamp<int>(static_cast<int>(std::lroundf(m_avg_color[2] * limit_f / 255.0f)), 0, m_limit);

    if (m_pParams->m_quality <= etc1_quality::Medium)
    {
        m_pSorted_luma_indices = indirect_radix_sort(n, m_sorted_luma[0].data(), m_sorted_luma[1].data(), m_luma.data(), 0, sizeof(m_luma[0]), false);
        m_pSorted_luma = m_sorted_luma[0].data();
        if (m_pSorted_luma_indices == m_sorted_luma[0].data()) {
            m_pSorted_luma = m_sorted_luma[1].data();
}
...
bool etc1_optimizer::evaluate_solution(const etc1_solution_coordinates& coords, potential_solution& trial_solution, potential_solution* pBest_solution)
{
    ...
    const rg_etc1_evaluate_inten_tables_func eval_inten_tables = get_cached_eval_inten_tables();

    std::array<u64, cETC1IntenModifierValues> table_errors;
    std::array<u8, static_cast<std::size_t>(cETC1IntenModifierValues) * 8> table_selectors;
    eval_inten_tables(m_pParams->m_pSrc_pixels, base_color, table_errors.data(), table_selectors.data());

#ifdef RG_ETC1_BUILD_DEBUG
    {
        std::array<u64, cETC1IntenModifierValues> scalar_errors;
        std::array<u8, cETC1IntenModifierValues * 8> scalar_selectors;
        evaluate_inten_tables_scalar(m_pParams->m_pSrc_pixels, base_color, scalar_errors.data(), scalar_selectors.data());
        RG_ETC1_ASSERT(memcmp(scalar_errors.data(), table_errors.data(), sizeof(table_errors)) == 0);
        RG_ETC1_ASSERT(memcmp(scalar_selectors.data(), table_selectors.data(), sizeof(table_selectors)) == 0);
    }
#endif

    for (u32 inten_table = 0; inten_table < cETC1IntenModifierValues; inten_table++)
    {
        const u64 total_error = table_errors[inten_table];

        if (total_error < trial_solution.m_error)
        {
            trial_solution.m_error = total_error;
            trial_solution.m_coords.m_inten_table = inten_table;
            std::memcpy(trial_solution.m_selectors.data(), &table_selectors[static_cast<size_t>(inten_table) * 8], 8);
...
bool etc1_optimizer::evaluate_solution_fast(const etc1_solution_coordinates& coords, potential_solution& trial_solution, potential_solution* pBest_solution)
{
    ...
    for (int inten_table = cETC1IntenModifierValues - 1; inten_table >= 0; --inten_table)
    {
        ...
        // 0   1   2   3
        //   01  12  23
        const std::array<u32, 3> block_inten_midpoints = { block_inten[0] + block_inten[1], block_inten[1] + block_inten[2], block_inten[2] + block_inten[3] };
        ...
        if ((m_pSorted_luma[n - 1] * 2) < block_inten_midpoints[0])
        {
            ...
            {
                const u32 min_error = block_inten[0] - m_pSorted_luma[n - 1];
                if (min_error >= trial_solution.m_error) {
                    continue;
}
            }

            std::memset(m_temp_selectors.data(), 0, n);
        ...
        else if ((m_pSorted_luma[0] * 2) >= block_inten_midpoints[2])
        {
            ...
            {
                const u32 min_error = m_pSorted_luma[0] - block_inten[3];
                if (min_error >= trial_solution.m_error) {
                    continue;
}
            }

            std::memset(m_temp_selectors.data(), 3, n);
        ...
        {
            u32 cur_selector = 0, c;
            for (c = 0; c < n; c++)
            {
                const u32 y = m_pSorted_luma[c];
                while ((y * 2) >= block_inten_midpoints[cur_selector]) {
                    if (++cur_selector > 2) {
                        goto done;
}
}
                const u32 sorted_pixel_index = m_pSorted_luma_indices[c];
                m_temp_selectors[sorted_pixel_index] = static_cast<u8>(cur_selector);
        ...
        if (total_error < trial_solution.m_error)
        {
            trial_solution.m_error = total_error;
            trial_solution.m_coords.m_inten_table = inten_table;
            std::memcpy(trial_solution.m_selectors.data(), m_temp_selectors.data(), n);
            trial_solution.m_valid = true;
            if (!total_error) {
                break;
}
...
static u32 etc1_decode_value(u32 diff, u32 inten, u32 selector, u32 packed_c)
{
    RG_ETC1_ASSERT((diff < 2) && (inten < 8) && (selector < 4) && (packed_c < (diff ? 32 : 16)));
    int c;
    if (diff) {
        c = static_cast<int>(packed_c >> 2) | static_cast<int>(packed_c << 3);
    } else {
        c = static_cast<int>(packed_c) | (static_cast<int>(packed_c) << 4);
}
...
void pack_etc1_block_init()
{
    for (u32 diff = 0; diff < 2; diff++)
    {
        ...
        for (u32 inten = 0; inten < 8; inten++)
        {
            ...
            {
                const u32 inverse_table_index = diff + (inten << 1) + (selector << 4);
                for (u32 color = 0; color < 256; color++)
                {
                    u32 best_error = cUINT32_MAX, best_packed_c = 0;
                    for (u32 packed_c = 0; packed_c < limit; packed_c++)
                    {
                        int v = static_cast<int>(etc1_decode_value(diff, inten, selector, packed_c));
                    ...
                    g_etc1_inverse_lookup[inverse_table_index][color] = static_cast<u16>(best_packed_c | (best_error << 8));
                }
            }
        }
    }
    ...
    for(int i = 0; i < 256 + 16; i++)
    {
        ...
        g_quant5_tab[i] = static_cast<u8>(expand5[mul_8bit(v,31)]);
    }

    g_eval_inten_tables = get_evaluate_inten_tables_func();
}

// Packs solid color blocks efficiently using a set of small precomputed tables.
// For random 888 inputs, MSE results are better than Erricson's ETC1 packer in "slow" mode ~9.5% of the time, is slightly worse only ~.01% of the time, and is equal the rest of the time.
static u64 pack_etc1_block_solid_color(etc1_block& block, const u8* pColor, [[maybe_unused]] etc1_pack_params& pack_params)
{
    RG_ETC1_ASSERT(g_etc1_inverse_lookup[0][255]);

    static const std::array<u32, 4> s_next_comp = { 1, 2, 0, 1 };

    u32 best_error = cUINT32_MAX, best_i = 0;
    ...
    for (u32 i = 0; i < 3; i++)
    {
        const u32 c1 = pColor[s_next_comp[i]], c2 = pColor[s_next_comp[i + 1]];
...
#ifdef RG_ETC1_BUILD_DEBUG
                const u32 diff = x & 1;
                const u32 inten = (x >> 1) & 7;
                const u32 selector = (x >> 4) & 3;
                const u32 p0 = (x >> 8) & 255;
                RG_ETC1_ASSERT(etc1_decode_value(diff, inten, selector, p0) == (u32)c_plus_delta);
#endif

                const u16* pInverse_table = g_etc1_inverse_lookup[x & 0xFF].data();
                u16 p1 = pInverse_table[c1];
                u16 p2 = pInverse_table[c2];
                const u32 trial_error = rg_etc1::square(c_plus_delta - pColor[i]) + rg_etc1::square(p1 >> 8) + rg_etc1::square(p2 >> 8);
                if (trial_error < best_error)
                {
                    best_error = trial_error;
...
found_perfect_match:
    ...
    const u32 diff = best_x & 1;
    const u32 inten = (best_x >> 1) & 7;

    block.m_bytes[3] = static_cast<u8>(((inten | (inten << 3)) << 2) | (diff << 1));

    const u32 etc1_selector = g_selector_index_to_etc1[(best_x >> 4) & 3];
    const u16 selector_words0 = static_cast<u16>((etc1_selector & 2) ? 0xFFFF : 0);
    const u16 selector_words1 = static_cast<u16>((etc1_selector & 1) ? 0xFFFF : 0);
    std::memcpy(&block.m_bytes[4], &selector_words0, sizeof(selector_words0));
    std::memcpy(&block.m_bytes[6], &selector_words1, sizeof(selector_words1));

    const u32 best_packed_c0 = (best_x >> 8) & 255;
    if (diff)
    {
        block.m_bytes[best_i] = static_cast<u8>(best_packed_c0 << 3);
        block.m_bytes[s_next_comp[best_i]] = static_cast<u8>(best_packed_c1 << 3);
        block.m_bytes[s_next_comp[best_i+1]] = static_cast<u8>(best_packed_c2 << 3);
    }
    else
    {
        block.m_bytes[best_i] = static_cast<u8>(best_packed_c0 | (best_packed_c0 << 4));
        block.m_bytes[s_next_comp[best_i]] = static_cast<u8>(best_packed_c1 | (best_packed_c1 << 4));
        block.m_bytes[s_next_comp[best_i+1]] = static_cast<u8>(best_packed_c2 | (best_packed_c2 << 4));
    }

    return best_error;
}

static u32 pack_etc1_block_solid_color_constrained(
    etc1_optimizer::results& results,
    u32 num_colors, const u8* pColor,
    [[maybe_unused]] etc1_pack_params& pack_params,
    bool use_diff,
    ...
    for (u32 i = 0; i < 3; i++)
    {
        ...
        for (int delta = -delta_range; delta <= delta_range; delta++)
        {
            ...
            for (;;)
            {
                const u32 x = *pTable++;
                const u32 diff = x & 1;
                if (static_cast<u32>(use_diff) != diff)
                {
                    if (*pTable == 0xFFFF) {
                        break;
}
                ...
                {
                    const int p0 = static_cast<int>((x >> 8) & 255);
...
found_perfect_match:
    ...
    const u32 best_packed_c0 = (best_x >> 8) & 255;
    results.m_block_color_unscaled[best_i] = static_cast<u8>(best_packed_c0);
    results.m_block_color_unscaled[s_next_comp[best_i]] = static_cast<u8>(best_packed_c1);
    results.m_block_color_unscaled[s_next_comp[best_i + 1]] = static_cast<u8>(best_packed_c2);
...
{
    const auto* pSrc_pixels = std::bit_cast<const color_quad_u8*>(pSrc_pixels_rgba);
    ...
    const u32 first_pixel_u32 = pSrc_pixels->m_u32;
    int r;
    for (r = 15; r >= 1; --r) {
    ...
    if (!r) {
        return static_cast<u32>(16 * pack_etc1_block_solid_color(dst_block, &pSrc_pixels[0].r, pack_params));
}

    std::array<color_quad_u8, 16> dithered_pixels;
    if (pack_params.m_dithering)
    {
        dither_block_555(dithered_pixels.data(), pSrc_pixels);
        pSrc_pixels = dithered_pixels.data();
    }

    etc1_optimizer optimizer;

    u64 best_error = cUINT64_MAX;
    u32 best_flip = false, best_use_color4 = false;

    std::array<std::array<u8, 8>, 2> best_selectors;
    std::array<etc1_optimizer::results, 2> best_results;
    for (u32 i = 0; i < 2; i++)
    {
        best_results[i].m_n = 8;
        best_results[i].m_pSelectors = best_selectors[i].data();
    }

    std::array<std::array<u8, 8>, 3> selectors;
    std::array<etc1_optimizer::results, 3> results;

    for (u32 i = 0; i < 3; i++)
    {
        results[i].m_n = 8;
        results[i].m_pSelectors = selectors[i].data();
    }

    std::array<color_quad_u8, 8> subblock_pixels;

    etc1_optimizer::params params(pack_params);
    params.m_num_src_pixels = 8;
    params.m_pSrc_pixels = subblock_pixels.data();

    for (u32 flip = 0; flip < 2; flip++)
    {
        for (u32 use_color4 = 0; use_color4 < 2; use_color4++)
        {
            u64 trial_error = 0;
            ...
            {
                if (flip) {
                    std::memcpy(subblock_pixels.data(), pSrc_pixels + static_cast<size_t>(subblock) * 8, sizeof(color_quad_u8) * 8);
                } else
                {
                    const color_quad_u8* pSrc_col = pSrc_pixels + static_cast<size_t>(subblock) * 2;
                ...
                if ((params.m_quality >= etc1_quality::Medium) && ((subblock) || (use_color4)))
                {
                    const u32 subblock_pixel0_u32 = subblock_pixels[0].m_u32;
                    for (r = 7; r >= 1; --r) {
                ...
                if (params.m_quality == etc1_quality::High)
                {
                    static const std::array<int, 9> s_scan_delta_0_to_4 = { -4, -3, -2, -1, 0, 1, 2, 3, 4 };
                    params.m_scan_delta_size = static_cast<u32>(std::size(s_scan_delta_0_to_4));
                    params.m_pScan_deltas = s_scan_delta_0_to_4.data();
                }
                else if (params.m_quality == etc1_quality::Medium)
                {
                    static const std::array<int, 3> s_scan_delta_0_to_1 = { -1, 0, 1 };
                    params.m_scan_delta_size = static_cast<u32>(std::size(s_scan_delta_0_to_1));
                    params.m_pScan_deltas = s_scan_delta_0_to_1.data();
                }
                else
                {
                    static const std::array<int, 1> s_scan_delta_0 = { 0 };
                    params.m_scan_delta_size = static_cast<u32>(std::size(s_scan_delta_0));
                    params.m_pScan_deltas = s_scan_delta_0.data();
                }
                ...
                if (params.m_quality >= etc1_quality::Medium)
                {
                    // TODO: Fix fairly arbitrary/unrefined thresholds that control how far away to scan for potentially better solutions.
                    const u32 refinement_error_thresh0 = 3000;
                    const u32 refinement_error_thresh1 = 6000;
                    if (results[subblock].m_error > refinement_error_thresh0)
                    {
                        if (params.m_quality == etc1_quality::Medium)
                        {
                            static const std::array<int, 4> s_scan_delta_2_to_3 = { -3, -2, 2, 3 };
                            params.m_scan_delta_size = static_cast<u32>(std::size(s_scan_delta_2_to_3));
                            params.m_pScan_deltas = s_scan_delta_2_to_3.data();
                        }
                        else
                        {
                            static const std::array<int, 2> s_scan_delta_5_to_5 = { -5, 5 };
                            static const std::array<int, 8> s_scan_delta_5_to_8 = { -8, -7, -6, -5, 5, 6, 7, 8 };
                            if (results[subblock].m_error > refinement_error_thresh1)
                            {
                                params.m_scan_delta_size = static_cast<u32>(std::size(s_scan_delta_5_to_8));
                                params.m_pScan_deltas = s_scan_delta_5_to_8.data();
                            }
                            else
                            {
                                params.m_scan_delta_size = static_cast<u32>(std::size(s_scan_delta_5_to_5));
                                params.m_pScan_deltas = s_scan_delta_5_to_5.data();
                            }
                        }
    ...
    if (best_use_color4)
    {
        dst_block.m_bytes[0] = static_cast<u8>(best_results[1].m_block_color_unscaled.r | (best_results[0].m_block_color_unscaled.r << 4));
        dst_block.m_bytes[1] = static_cast<u8>(best_results[1].m_block_color_unscaled.g | (best_results[0].m_block_color_unscaled.g << 4));
        dst_block.m_bytes[2] = static_cast<u8>(best_results[1].m_block_color_unscaled.b | (best_results[0].m_block_color_unscaled.b << 4));
    }
    else
    {
        if (dr < 0) { dr += 8;
}
        dst_block.m_bytes[0] = static_cast<u8>((best_results[0].m_block_color_unscaled.r << 3) | dr);
        if (dg < 0) { dg += 8;
}
        dst_block.m_bytes[1] = static_cast<u8>((best_results[0].m_block_color_unscaled.g << 3) | dg);
        if (db < 0) { db += 8;
}
        dst_block.m_bytes[2] = static_cast<u8>((best_results[0].m_block_color_unscaled.b << 3) | db);
    }

    dst_block.m_bytes[3] = static_cast<u8>( (best_results[1].m_block_inten_table << 2) | (best_results[0].m_block_inten_table << 5) | ((~best_use_color4 & 1) << 1) | best_flip );
    ...
    if (best_flip)
    {
        ...
        // { 0, 2 }, { 1, 2 }, { 2, 2 }, { 3, 2 },
        // { 0, 3 }, { 1, 3 }, { 2, 3 }, { 3, 3 }
        const u8* pSelectors0 = best_results[0].m_pSelectors;
        const u8* pSelectors1 = best_results[1].m_pSelectors;
        for (int x = 3; x >= 0; --x)
        {
            u32 b;
    ...
    else
    {
        ...
        {
            const u8* pSelectors = best_results[subblock].m_pSelectors + 4;
            for (u32 i = 0; i < 2; i++)
            {
                u32 b;
    ...
    dst_block.m_bytes[4] = static_cast<u8>(selector1 >> 8); dst_block.m_bytes[5] = static_cast<u8>(selector1 & 0xFF);
    dst_block.m_bytes[6] = static_cast<u8>(selector0 >> 8); dst_block.m_bytes[7] = static_cast<u8>(selector0 & 0xFF);

    return static_cast<u32>(best_error);
}
